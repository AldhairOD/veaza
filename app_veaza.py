...
def validar_producto(d: dict) -> Tuple[bool,str]:
    ...
    if len(d.get("moneda","")) != 3:
        return False, "moneda debe ser código ISO de 3 letras (PEN, USD, ...)."
    return True, ""
...
can_opts = _opts("— Selecciona —", {str(c["_id"]): c.get("codigo") for c in CANS})
...
with tabs[0]:
    ...
    texto = st.text_input("Buscar por código/nombre", "")
    ...
    st.dataframe(pd.DataFrame(rows).assign(_id=lambda d: d["_id"].astype(str)), use_container_width=True)

    st.markdown("—")
    with st.form("can_create", clear_on_submit=True):
        c_codigo = st.text_input("Código (ej. WEB)")
        c_nombre = st.text_input("Nombre")
        c_tipo   = st.selectbox("Tipo", ["TIENDA","WEB","APP","DELIVERY","PICKUP"])
        c_activo = st.checkbox("Activo", value=True)
        if st.form_submit_button("Crear canal"):
            ...
            if ok:
                try:
                    ...
                    st.error("❌ Canal duplicado (revisa índice único por 'codigo').")
...
with tabs[1]:
    ...
    txt = st.text_input("Buscar por nombre/ciudad", "")
    ...
    with st.form("ubi_create", clear_on_submit=True):
        ...
        u_tipo   = st.selectbox("Tipo ubicación", ["TIENDA_FISICA","ALMACEN","DARK_STORE"])
        u_pais   = st.text_input("País", value="Perú")
        u_ciudad = st.text_input("Ciudad", value="")
        u_dir    = st.text_input("Dirección", value="")
        u_activo = st.checkbox("Activo", value=True)
        if st.form_submit_button("Crear ubicación"):
...
with tabs[2]:
    ...
    with st.form("cat_create", clear_on_submit=True):
        c_nombre = st.text_input("Nombre")
        c_slug   = st.text_input("Slug")
        c_parent = st.selectbox("Parent", list(cat_opts.keys()))
        ...
        if st.form_submit_button("Crear categoría"):
            ...
            if ok:
                try:
                    ...
                    st.error("❌ Slug duplicado (revisa índice único).")
                except Exception as e:
                    st.error(f"❌ Error: {e}")
            else:
                st.error(msg)
...
with tabs[3]:
    ...
    with colp1: pt = st.text_input("Buscar (sku/nombre/desc.)", "")
    with colp2: pe = st.selectbox("Estado", ["— Todos —","ACTIVO","INACTIVO","DESCONTINUADO"])
    with colp3:
        pcat_lbl = st.selectbox("Categoría", ["— Todos —"] + list(cat_map.values()))
    ...
    with st.form("prod_create", clear_on_submit=True):
        sku = st.text_input("SKU"); nombre = st.text_input("Nombre")
        desc = st.text_area("Descripción", value="")
        c_lbl = st.selectbox("Categoría", list(cat_opts.keys()))
        c_val = cat_opts[c_lbl]
        precio = st.text_input("Precio", value="")
        moneda = st.text_input("Moneda (ISO 3)", value="PEN")
        estado = st.selectbox("Estado", ["ACTIVO","INACTIVO","DESCONTINUADO"])
...
with tabs[4]:
    ...
    with st.form("inv_upsert", clear_on_submit=True):
        ...
        reservado = st.number_input("Reservado", min_value=0, value=0)
        seg = st.number_input("Stock de seguridad", min_value=0, value=0)
...
with tabs[5]:
    ...
    c_txt = st.text_input("Buscar (nombres, apellidos, doc, correo)", "")
    ...
    with st.form("cli_create", clear_on_submit=True):
        tdoc = st.selectbox("Tipo doc.", ["DNI","CE","PAS"])
        dnum = st.text_input("N° documento")
        nom  = st.text_input("Nombres")
        ape  = st.text_input("Apellidos")
        cor  = st.text_input("Correo", value="")
        tel  = st.text_input("Teléfono", value="")
        dirc = st.text_input("Dirección", value="")
        segm = st.selectbox("Segmento", ["REGULAR","VIP","NUEVO"], index=0)
...
with tabs[6]:
    ...
    # Buscar carrito por cliente+canal
    cc_lbl = st.selectbox("Cliente", list(cli_opts.keys()))
    cc_val = cli_opts[cc_lbl]
    canal_codigo = st.selectbox("Canal", ["WEB","APP","TIENDA","DELIVERY","PICKUP"], index=0)
    if cc_val:
    ...
    n_items = st.number_input("N° de ítems", min_value=1, max_value=10, value=1)
    items = []
    ...
    if cart:
        ...
        st.json({**cart, "_id": str(cart["_id"])}, expanded=False)
...
with tabs[7]:
    ...
    o_txt = st.text_input("Buscar por código", "")
    o_estado = st.selectbox("Estado", ["— Todos —","CREADA","PAGADA","PREPARACION","EN_RUTA","LISTA_RECOJO","ENTREGADA","CANCELADA","DEVUELTA"])
    filt = {}
    ...
    oc_canal = st.selectbox("Canal", ["WEB","APP","TIENDA","DELIVERY","PICKUP"], index=0)
    oc_moneda = st.text_input("Moneda (ISO 3)", value="PEN")
    n = st.number_input("N° de ítems", min_value=1, max_value=10, value=1)
    o_items = []
    for i in range(int(n)):
        p_lbl = st.selectbox(f"Producto #{i+1}", list(prod_opts.keys()), key=f"ord_p_{i}")
        p_val = prod_opts[p_lbl]
        qty   = st.number_input(f"Cantidad #{i+1}", min_value=1, value=1, key=f"ord_q_{i}")
        price = float(precio_by_id.get(p_val,0.0)) if p_val else 0.0
...
with tabs[8]:
    st.subheader("💳 Pagos")
    ord_code = st.text_input("Código de orden", "")
    ...
    if o:
        ...
        with st.form("pay_create", clear_on_submit=True):
            monto = st.text_input("Monto", value=str(o.get("total","")))
            moneda = st.text_input("Moneda (ISO 3)", value=o.get("moneda","PEN"))
            metodo = st.selectbox("Método", ["TARJETA","YAPE","PLIN","TRANSFERENCIA","EFECTIVO"])
            estado = st.selectbox("Estado", ["PENDIENTE","APROBADO","RECHAZADO","REEMBOLSADO"])
    ...
    } for r in pagos_rows]), use_container_width=True)
...
with tabs[9]:
    ...
    if o2:
        with st.form("ship_create", clear_on_submit=True):
            tipo = st.selectbox("Tipo", ["DELIVERY","PICKUP"])
            estado = st.selectbox("Estado", ["PENDIENTE","PREPARANDO","LISTO","EN_RUTA","ENTREGADO","CANCELADO"])
            proveedor = st.text_input("Proveedor", value="Veaza Logistics")
            tracking  = st.text_input("Tracking", value=f"VL-{o2.get('codigo')}")
            if st.form_submit_button("Crear envío"):
                ...
                if ok:
                    # un envío por orden (índice único sugerido)
    ...
    st.dataframe(pd.DataFrame([{
        ...
        "Actualizado": r.get("actualizado_en").isoformat() if r.get("actualizado_en") else ""
    } for r in env_rows]), use_container_width=True)
...
with tabs[10]:
    ...
    txt = st.text_input("Buscar por código/desc.", "")
    ...
    with st.form("promo_create", clear_on_submit=True):
        pc = st.text_input("Código"); pdsc = st.text_input("Descripción")
        pt = st.selectbox("Tipo", ["PCT_DESC","MONTO_DESC","ENVIO_GRATIS"])
        pv = st.text_input("Valor (opcional)")
        pact = st.checkbox("Activo", value=True)
        if st.form_submit_button("Crear promoción"):
...
with tabs[11]:
    ...
    estado_r = st.selectbox("Estado", ["SOLICITADA","APROBADA","RECHAZADA","RECIBIDA","REEMBOLSADA","CERRADA"])
    if st.button("Crear devolución"):
...
with tabs[12]:
    ...
    tipo = st.selectbox("Tipo", ["— Todos —","ORDER_CREATED","ORDER_PAID","STOCK_UPDATED","SHIPMENT_STATUS","PROMO_APPLIED","CART_UPDATED"])
...
st.caption("✅ Esta app usa TODAS las colecciones del caso: canales, ubicaciones, categorias, productos, inventario, clientes, carritos, ordenes, pagos, envios, promociones, devoluciones y eventos.")
